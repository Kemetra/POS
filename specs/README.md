...
| [022-pos-ui-v4-rescue](./022-pos-ui-v4-rescue/) | Active | POS UI v4.0 visual convergence. Spec chain complete (specify→clarify→plan→tasks→analyze). **Implementation in progress:** **US4a (sale-success honesty) MERGED** — the settled phase now renders two truthful states (payment-taken vs sale-finalized) instead of an unconditional English success claim. **U0 (visual foundation) IN PROGRESS** — light default, pharmacy-teal `#0f766e` primary (dark register retuned to `#14b8a6`, T024 option (a)), Arabic-first system font stack, and a hard-zero five-family design-token guard. Establishes v4.0 as the governing **visual** authority, superseding POS v3.5 on three visual axes (banners added to the v3.5 docs); v3.5 remains valid implementation/structural context. Visual-only except one approved role-aware `/app` landing correction (cashier → `/app/cart`, not yet implemented). **Screenshot evidence (T002/T0A2/T0B2) still OPEN** — requires a human-observed Electron launch |
