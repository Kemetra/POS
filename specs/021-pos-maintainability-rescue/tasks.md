...
- [ ] **T030** [S3] Author `src/main/app/__tests__/bootstrap-window.test.ts` FIRST, asserting the
...
- [ ] **T031** [S3] Test: `will-navigate` denies any URL outside the resolved renderer origin.
- [ ] **T032** [S3] Test: `setWindowOpenHandler` denies all new-window requests.
- [ ] **T033** [S3] Test: **both** CSP strings (dev and production) are character-identical to current.
      Any weakening must red this test.
- [ ] **T034** [S3] Create `src/main/app/bootstrap-window.ts` and move `createWindow()` (`:202–:262`)
      **verbatim**, including the navigation allow-list, window-open deny, both CSP layers, and the
      dev-vs-packaged load path.
- [ ] **T035** [S3] **AD-4 check.** Verify `bootstrap-wires-operator.test.ts`; if the move relocated any
...
- [ ] **T036** [S3] Run full gates. All green.
- [ ] **T037** [S3] Diff review against SC-4 and NFR-5 (no security value altered).
...
- [ ] **T040** [S2] Author `src/main/app/__tests__/bootstrap-db.test.ts` FIRST, asserting current
...
- [ ] **T041** [S2] Create `src/main/app/bootstrap-db.ts` owning open + migrate + close mechanics only.
...
- [ ] **T042** [S2] Move the `dbHandle` declaration and its close block into the new module's ownership,
      preserving: closed exactly once, **after** `stopAll()` (plan AD-2), guarded against double-close.
- [ ] **T043** [S2] **Explicitly NOT in scope** — verify the diff touches none of: repositories,
      transaction semantics, WAL/pragma values, migration runner logic, append-only triggers,
      `migrations/`.
- [ ] **T044** [S2] **AD-4 check** on `bootstrap-wires-operator.test.ts` (most likely slice to trip it).
- [ ] **T045** [S2] Run full gates. All green.
- [ ] **T046** [S2] Confirm FR-2 fully discharged: **no** process-lifetime mutable state declared
...
- [ ] **T050** [P] [S4] Rename `CartPlaceholder` → behavioral name. Evidence: mounts live `CartPane` +
      `CatalogueSalePane`. Update coupled tests **in this slice**.
- [ ] **T051** [P] [S4] Rename `SalesPlaceholder` → behavioral name. Evidence: mounts live
      `FindSaleReceipt` (a real reprint feature). Update coupled tests in this slice.
- [ ] **T052** [P] [S4] Rename `CheckoutPlaceholder` → behavioral name. Evidence: **live** — rendered at
...
- [ ] **T053** [S4] Rename `src/main/payments/voucher-authority/` → a client-accurate name. Evidence: it
...
- [ ] **T054** [S4] **Out of scope — do NOT rename:** `EmptyCartPlaceholder`, `DiscountPlaceholderRow`,
...
- [ ] **T055** [S4] Verify no route path string, feature-flag default, or exported cross-boundary
      contract changed (FR-8).
- [ ] **T056** [S4] Run full gates after each sub-slice.
...
- [ ] **T060** [S5] **Evidence gate.** `sales-sync/` = 5 files; `sync-outbox/` = 1 repository file.
...
- [ ] **T061** [S5] If the gate passes: move the single file, preserving all import contracts. No runtime
      wiring change. Full gates green.
- [ ] **T062** [S6] **Evidence gate.** `CatalogueDiagnostics` is already routed separately at
...
- [ ] **T063** [S6] If the gate passes: clarify labeling/grouping only. **Do NOT** redesign diagnostics
      or any cashier UI.
...
- [ ] **T070** [P] [S7] Create `docs/architecture/current.md` — the internal POS architecture and
...
- [ ] **T071** [P] [S7] Create `specs/README.md` — a lightweight status index classifying every spec
      directory as **Active**, **Implemented / Historical**, **Deferred**, or **Superseded**
      ("Superseded" only where evidence supports it).
- [ ] **T072** [S7] Correct the stale closing line of `docs/architecture/synchronization.md` that
...
- [ ] **T073** [S7] **Prohibitions check.** Verify the diff: no `specs/<id>/` directory relocated or
...
- [ ] **T074** [S7] Verify SC-8a: no remaining document points at `specs/**` as the authority for
